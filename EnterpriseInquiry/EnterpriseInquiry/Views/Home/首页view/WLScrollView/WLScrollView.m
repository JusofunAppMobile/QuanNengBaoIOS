...
- (NSArray *)checkIndexAry{
    ...
    NSInteger index_0 = _index - MIN(2, _cellNum);//2不能越界
    NSInteger index_1 = _index - MIN(1, _cellNum);
    NSInteger index_2 = _index;
    NSInteger index_3 = _index + MIN(1, _cellNum);
    NSInteger index_4 = _index + MIN(2, _cellNum);
    if (index_1 < 0) {
...
- (void)didSelectedRespond:(id)data{
    if (self.delegate && [self.delegate respondsToSelector:@selector(scrollView:didSelectedAtIndex:data:)]) {
        [self.delegate scrollView:self didSelectedAtIndex:_index data:data];
    }else{
