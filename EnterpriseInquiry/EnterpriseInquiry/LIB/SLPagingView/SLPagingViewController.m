...
@property (nonatomic, strong) UIScrollView *slScrollView;
    ...
    if(self){
        ...
        for(i=0; i<items.count; i++){
            ...
            if([[items objectAtIndex:i] isKindOfClass:UIView.class])
                [self addNavigationItem:[items objectAtIndex:i] tag:i count:(int)items.count];
            
//            if (i == items.count-1) {
//                
//                _lastBarRightItemImageV = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"登出"]];
//                _lastBarRightItemImageV.frame = CGRectMake(0, 0, 24, 24);
//
//                _lastBarRightItemImageV.frame = (CGRect){SCREEN_SIZE.width-35, 28, 25, 25};
//                UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self
//                                                                                      action:@selector(tapOnHeader:)];
//                [_lastBarRightItemImageV addGestureRecognizer:tap];
//                [_lastBarRightItemImageV setUserInteractionEnabled:YES];
//                [_navigationBarView addSubview:_lastBarRightItemImageV];
//                
//            }else{
//            
//                _lastBarRightItemImageV.hidden = YES;
//            }
        }
...
//                                               object:nil];
    ...
    self.slScrollView.delegate = self;
}
...
-(void)viewWillDisappear:(BOOL)animated{
    ...
    self.slScrollView.delegate = nil;
...
- (void)viewWillLayoutSubviews {
    ...
    self.navigationBarView.frame = (CGRect){0, -20, SCREEN_SIZE.width, KNavigationBarHeight};
...
-(void)setCurrentIndex:(NSInteger)index animated:(BOOL)animated{
    ...
    [self.slScrollView setContentOffset:CGPointMake(xOffset, self.slScrollView.contentOffset.y) animated:animated];
}
...
-(void)addViewControllers:(UIViewController *) controller needToRefresh:(BOOL) refresh{
    ...
    [self addNavigationItem:v tag:tag count:(int)self.viewControllers.count];
    // Save the controller
...
// 传入一个count 做比较 --- create by LEZ  16/9/23
-(void)addNavigationItem:(UIView *)v tag:(int)tag count:(int)count{
    
    
    CGFloat imageY = KStatusBarHeight+ 44/2-20;
    if(KIsIphoneX)
    {
        imageY = 22;
    }
    ...
    imageV.frame = (CGRect){originX, imageY, 40, 40};
    ...
    [_navigationBarView addSubview:imageV];
    if(!_navItemsViews)
        _navItemsViews = [[NSMutableArray alloc] init];
    [_navItemsViews addObject:imageV];
}

-(void)setupPagingProcess{
    // Make our ScrollView
    CGRect frame                                              = CGRectMake(0, 0, SCREEN_SIZE.width, self.view.bounds.size.height);
    self.slScrollView                                           = [[UIScrollView alloc] initWithFrame:frame];
    self.slScrollView.backgroundColor                           = [UIColor clearColor];
    self.slScrollView.pagingEnabled                             = YES;
    self.slScrollView.showsHorizontalScrollIndicator            = NO;
    self.slScrollView.showsVerticalScrollIndicator              = NO;
    self.slScrollView.delegate                                  = self;
    self.slScrollView.bounces                                   = NO;
    [self.slScrollView setContentInset:UIEdgeInsetsMake(0, 0, -80, 0)];
    [self.view addSubview:self.slScrollView];
...
-(void)addControllers{
    if(self.viewControllers
       && self.viewControllers.count > 0){
        ...
        self.slScrollView.contentSize = (CGSize){width, height};
        __block int i = 0;
        ...
        [sortedIndexes enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            ...
            [self.slScrollView addSubview:v];
            if([self useAutoLayout:v]){
                // Using AutoLayout
                v.translatesAutoresizingMaskIntoConstraints = NO;
                // Width constraint, half of parent view width
                [self.slScrollView addConstraint:[NSLayoutConstraint constraintWithItem:v
                                                                            attribute:NSLayoutAttributeWidth
                                                                            relatedBy:NSLayoutRelationEqual
                                                                               toItem:self.slScrollView
                                                                            attribute:NSLayoutAttributeWidth
                                                                           multiplier:1.0
                                                                             constant:0]];
                ...
                if(previous)
                    ...
                    [self.slScrollView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:[previous]-0-[v]"
                                                                                            options:0
                                                                                            metrics:nil
                                                                                              views:@{@"v" : v, @"previous" : previous}]];
                else
                    // First view
                    [self.slScrollView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-0-[v]"
                                                                                            options:0
                                                                                            metrics:nil
                                                                                              views:@{@"v" : v }]];
                // Oridnate constraint : set the space between the Top and the current view
                [self.slScrollView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:[NSString stringWithFormat:@"V:|-%f-[v]", CGRectGetHeight(self.navigationBarView.frame)]
                                                                                        options:0
                                                                                        metrics:nil
                                                                                          views:@{@"v" : v}]];
                }
...
-(void)tapOnHeader:(UITapGestureRecognizer *)recognizer{
    if(self.isUserInteraction){
        ...
        [self.slScrollView scrollRectToVisible:view.frame
                                    animated:YES];
    }
}
...
- (void)goToLogin{
    UIView *view = [self.viewControllers objectForKey:@(2)];
    [self.slScrollView scrollRectToVisible:view.frame
                                  animated:YES];
}
...
-(void)rightBarItemClick
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(rightBarItemClicked)]) {
        [self.delegate rightBarItemClicked];
    }
}
...
-(void)adaptViews{
    ...
    [self updateNavItems:self.slScrollView.contentOffset.x];
    // Be sure to stay on the same view
    [self setCurrentIndex:self.indexSelected
                 animated:NO];
    [self.slScrollView setNeedsUpdateConstraints];
...
-(void)updateContentSize{
    float nWidth     = SCREEN_SIZE.width * self.slScrollView.subviews.count;
    float nHeight    = (self.slScrollView.subviews.count > 0) ? CGRectGetHeight([(UIView*)self.slScrollView.subviews[0] frame]):CGRectGetHeight(self.slScrollView.frame);
    self.slScrollView.contentSize = (CGSize){nWidth, nHeight};
}
...
-(void)scrollViewWillBeginDragging:(UIScrollView *)scrollView{
    // Try to notify the controller concerned
    if(scrollView == self.slScrollView)
    {
        [self notifyControllers:NSSelectorFromString(@"viewWillDisappear:")
                         object:@(YES)
                     checkIndex:YES
